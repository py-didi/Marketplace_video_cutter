...
class VideoProcessorApp:
    def __init__(self, root):
        ...
        self.is_completed = False  # Флаг для предотвращения многократного завершения
    ...
    def process_single_video(self, filename, input_folder, output_folder, start_time, end_time, target_width, target_height):
        ...
        if width is None or height is None or duration is None:
            ...
            return f"Ошибка: Не удалось получить информацию о видео '{filename}'"

        # Проверка длительности
        if duration < start_time:
            logging.warning(f"Видео {filename} короче времени обрезки: {duration} < {start_time}")
            return f"Ошибка: Видео '{filename}' короче времени обрезки ({start_time} сек)"
        ...
        # Рассчитываем масштабирование так, чтобы обе стороны были не меньше целевых
        input_aspect = effective_width / effective_height
        target_aspect = target_width / target_height

        if input_aspect > target_aspect:
            # Входное видео шире, чем целевое
            scale_height = int(target_width / input_aspect)
            scale_width = target_width
        else:
            # Входное видео выше, чем целевое
            scale_width = int(target_height * input_aspect)
            scale_height = target_height

        # Убедимся, что масштабированные размеры достаточно велики
        if scale_width < target_width or scale_height < target_height:
            scale_factor = max(target_width / effective_width, target_height / effective_height)
            scale_width = int(effective_width * scale_factor)
            scale_height = int(effective_height * scale_factor)
        ...
        # Обрезка до точного целевого размера
        ...
        try:
            ...
            if result.returncode == 0:
                if os.path.exists(output_path):
                    ...
                    return f"Успешно обработано: {output_path} ({file_size_mb:.2f} МБ)"
                else:
                    logging.error(f"Файл {output_path} не был создан после FFmpeg")
                    return f"Ошибка: Файл {output_path} не создан после FFmpeg"
            else:
                error_message = result.stderr
                logging.error(f"Ошибка FFmpeg: {error_message}")
                return f"Ошибка FFmpeg для {filename}: {error_message}"
        except Exception as e:
            logging.error(f"Исключение в FFmpeg: {str(e)}")
            return f"Исключение в FFmpeg для {filename}: {str(e)}"

    def update_progress(self, result):
        """Обновляет прогресс и статус в интерфейсе."""
        if self.is_completed:
            return  # Пропускаем, если обработка уже завершена

        self.completed_files += 1
        self.progress_var.set((self.completed_files / self.video_files_count) * 100)
        self.status_var.set(result)

        if self.completed_files == self.video_files_count:
            self.is_completed = True
            self.status_var.set("Обработка завершена!")
            self.progress_var.set(100)
            self.root.after(0, lambda: messagebox.showinfo("Успех", "Обработка видео завершена!"))
    ...
    def process_videos(self):
        ...
        self.video_files_count = len(video_files)
        self.completed_files = 0
        self.is_completed = False  # Сбрасываем флаг завершения
        self.files_count_var.set(f"Найдено файлов: {self.video_files_count}")
        ...
        max_workers = min(multiprocessing.cpu_count(), 4)  # Ограничиваем количеством ядер или 4
        logging.info(f"Запуск обработки с {max_workers} потоками")
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {
                executor.submit(
                    ...
                    target_width,
                    target_height
                ): filename for filename in video_files
            }
            for future in as_completed(futures):
                filename = futures[future]
                try:
                    result = future.result()
                    self.root.after(0, self.update_progress, result)
                except Exception as e:
                    self.root.after(0, self.update_progress, f"Ошибка обработки {filename}: {str(e)}")
                    logging.error(f"Ошибка обработки {filename}: {str(e)}")
